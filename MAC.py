...
class MetricAnalyzer(RabbitMQConnection):
    ...
    def detect_issues(self, metrics: Dict[str, float]) -> None:
        ...
        if metrics['uptime'] > 86400 and not self.alert_states["high_uptime"]:
            self.send_notification("System has been up for over 24 hours!")
            self.alert_states["high_uptime"] = True
        elif metrics['uptime'] <= 86400 and self.alert_states["high_uptime"]:
            self.alert_states["high_uptime"] = False
